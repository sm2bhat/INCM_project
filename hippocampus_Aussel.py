...
def plot_activity(spikemon, defn, n_run):

    PATH = './neuronSpikesAll_inh/' + defn
    ...
    ylabel('Neuron index')
    title(defn)
    plt.savefig(PATH + defn + n_run + '.png')
...
def preparation(num_simu,g_max_e,g_max_i,p_co,p_co_CA3):
    ...
    global V_th, N1, N2, N3, p_CAN, asleep, grid, elec_ind, scale, runtime, tstp, sigma, coeff, scale, scale_str
    global noise_amp
    ...
    Vm = (- I_leak - I_K - I_Na) / ((1 * ufarad * cm ** -2) * (14e3 * umetre ** 2))*tstp : volt 
    ...
    # Pyramidal CAN
    # Conductance depends on sleep/wake
    global gCAN, gM
    if co=='sleep':
    ...
    Vm =( - I_CAN - I_M - I_leak - I_K - I_Na - I_Ca) / ((1 * ufarad * cm ** -2) * (29e3 * umetre ** 2))*tstp : volt 
    ...
    Vm=( - I_M - I_leak - I_K - I_Na - I_Ca) / ((1 * ufarad * cm ** -2) * (29e3 * umetre ** 2))*tstp : volt 
    I_M = ((gM) * (29e3 * umetre ** 2)) * p * (v - (-100 * mV)) : amp
        dp/dt = (pInf - p) / pTau : 1
        pInf = 1. / (1 + exp(- (v + (35 * mV)) / (10 * mV))) : 1
        pTau = (1000 * ms) / (3.3 * exp((v + (35 * mV)) / (20 * mV)) + exp(- (v + (35 * mV)) / (20 * mV))) : second
...
def process(num_simu,g_max_e,g_max_i,p_co,p_co_CA3) :
    ...
    input_S_1=read_file('input_files/poisson_samples_01.txt')
    input_S_2=read_file('input_files/poisson_samples_02.txt')
    input_S_3=read_file('input_files/poisson_samples_03.txt')
    
    input_W_1=read_file('input_files/poisson_samples_01.txt')
    input_W_2=read_file('input_files/poisson_samples_02.txt')
    input_W_3=read_file('input_files/poisson_samples_03.txt')
...
#    preparation(num_simu,g_max_e,g_max_i,p_co,p_co_CA3)
    ...
    In_exc1=NeuronGroup(10000, 'rates : Hz', threshold='rand()<inputs1(t)*tstp')    #dt ? record_dt ?
    S11 = Synapses(In_exc1, EC_py_CAN, on_pre='he_post+=g_max_e')
    S11.connect(p=p_in)
    S13 = Synapses(In_exc1, EC_inh, on_pre='he_post+=g_max_e')
    S13.connect(p=p_in)

    In_exc2=NeuronGroup(10000, 'rates : Hz', threshold='rand()<inputs2(t)*tstp')    #dt ? record_dt ?
    S21 = Synapses(In_exc2, EC_py_CAN, on_pre='he_post+=g_max_e')
    S21.connect(p=p_in)
    S23 = Synapses(In_exc2, EC_inh, on_pre='he_post+=g_max_e')
    S23.connect(p=p_in)

    In_exc3=NeuronGroup(10000, 'rates : Hz', threshold='rand()<inputs3(t)*tstp')    #dt ? record_dt ?
    ...
    signal_principal=zeros(int(runtime/tstp))

    
    for test_ind in range(nb_runs):
        prefs.codegen.target = 'numpy'
        print("--1")
        syn_CA3_py_CAN_E = StateMonitor(CA3_py_CAN,'I_SynE',record=True,dt=tstp)
        syn_CA1_py_CAN_E = StateMonitor(CA1_py_CAN,'I_SynE',record=True,dt=tstp)
        syn_DG_py_E = StateMonitor(DG_py,'I_SynE',record=True,dt=tstp)
        syn_EC_py_CAN_E = StateMonitor(EC_py_CAN,'I_SynE',record=True,dt=tstp)
        
        spikemon_CA3_py_CAN_E = SpikeMonitor(CA3_py_CAN,'I_SynE',record=True)
        spikemon_CA1_py_CAN_E = SpikeMonitor(CA1_py_CAN,'I_SynE',record=True)
        spikemon_DG_py_E = SpikeMonitor(DG_py,'I_SynE',record=True)
        spikemon_EC_py_CAN_E = SpikeMonitor(EC_py_CAN,'I_SynE',record=True)

        syn_CA3_py_CAN_Ext = StateMonitor(CA3_py_CAN,'I_SynExt',record=True,dt=tstp)
        syn_CA1_py_CAN_Ext = StateMonitor(CA1_py_CAN,'I_SynExt',record=True,dt=tstp)
        syn_DG_py_Ext = StateMonitor(DG_py,'I_SynExt',record=True,dt=tstp)
        syn_EC_py_CAN_Ext = StateMonitor(EC_py_CAN,'I_SynExt',record=True,dt=tstp)

        spikemon_syn_CA3_py_CAN_Ext = SpikeMonitor(CA3_py_CAN,'I_SynExt',record=True)
        spikemon_syn_CA1_py_CAN_Ext = SpikeMonitor(CA1_py_CAN,'I_SynExt',record=True)
        spikemon_syn_DG_py_Ext = SpikeMonitor(DG_py,'I_SynExt',record=True)
        spikemon_syn_EC_py_CAN_Ext = SpikeMonitor(EC_py_CAN,'I_SynExt',record=True)
        
        syn_CA3_py_CAN_I = StateMonitor(CA3_py_CAN,'I_SynI',record=True,dt=tstp)
        syn_CA1_py_CAN_I = StateMonitor(CA1_py_CAN,'I_SynI',record=True,dt=tstp)
        syn_DG_py_I = StateMonitor(DG_py,'I_SynI',record=True,dt=tstp)
        syn_EC_py_CAN_I = StateMonitor(EC_py_CAN,'I_SynI',record=True,dt=tstp)

        spikemon_syn_CA3_py_CAN_I = SpikeMonitor(CA3_py_CAN,'I_SynI',record=True)
        spikemon_syn_CA1_py_CAN_I = SpikeMonitor(CA1_py_CAN,'I_SynI',record=True)
        spikemon_syn_DG_py_I = SpikeMonitor(DG_py,'I_SynI',record=True)
        spikemon_syn_EC_py_CAN_I = SpikeMonitor(EC_py_CAN,'I_SynI',record=True)        
        
        run(single_runtime,report='text',report_period=1.00000*second)
        test_ind = str(test_ind)
        plot_activity(spikemon_CA3_py_CAN_E, 'CA3_py_CAN_E',test_ind)
        plot_activity(spikemon_CA1_py_CAN_E, 'CA1_py_CAN_E',test_ind)
        plot_activity(spikemon_DG_py_E, 'DG_py_E',test_ind)
        plot_activity(spikemon_EC_py_CAN_E, 'EC_py_CAN_E',test_ind)
        plot_activity(spikemon_syn_CA3_py_CAN_Ext, 'CA3_py_CAN_Ext',test_ind)
        plot_activity(spikemon_syn_CA1_py_CAN_Ext, 'CA1_py_CAN_Ext',test_ind)
        plot_activity(spikemon_syn_DG_py_Ext, 'DG_py_Ext',test_ind)
        plot_activity(spikemon_syn_EC_py_CAN_Ext, 'EC_py_CAN_Ext',test_ind)
        plot_activity(spikemon_syn_CA3_py_CAN_I, 'CA3_py_CAN_I',test_ind)
        plot_activity(spikemon_syn_CA1_py_CAN_I, 'CA1_py_CAN_I',test_ind)
        plot_activity(spikemon_syn_DG_py_I, 'DG_py_I',test_ind)
        plot_activity(spikemon_syn_EC_py_CAN_I, 'EC_py_CAN_I',test_ind)
        test_ind = int(test_ind)
        ###Calcul du LFP
        print('Calcul du LFP')  
        start_plot_time=500.0*msecond
        start_ind=int(start_plot_time/record_dt)      
        

        all_isyn=zeros((len(elec_pos),int(single_runtime/tstp)))
        lfp_dg_e=zeros((len(elec_pos),int(single_runtime/tstp)))
        lfp_dg_i=zeros((len(elec_pos),int(single_runtime/tstp)))
        lfp_ec_e=zeros((len(elec_pos),int(single_runtime/tstp)))
        lfp_ec_i=zeros((len(elec_pos),int(single_runtime/tstp)))
        lfp_ca1_e=zeros((len(elec_pos),int(single_runtime/tstp)))
        lfp_ca1_i=zeros((len(elec_pos),int(single_runtime/tstp)))
        lfp_ca3_e=zeros((len(elec_pos),int(single_runtime/tstp)))
        lfp_ca3_i=zeros((len(elec_pos),int(single_runtime/tstp)))
        ...
        del cos_angle
        del dx,dy,dz,dist
        signal_principal[test_ind*int(single_runtime/tstp):(test_ind+1)*int(single_runtime/tstp)]=isyn_principal
...
def main_process(simu_range,g_max_e,g_max_i,p_co,p_co_CA3):
    ...
    for num_simu in simu_range:
        all_results += (process(num_simu,g_max_e,g_max_i,p_co,p_co_CA3)).tolist()
